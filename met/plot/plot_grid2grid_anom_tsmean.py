...
while s <= nstats: #loop over statistics
   ...
   if stat_now_name == 'ac':
      ...
      while n <= nexp: #loop over experiments
         #get file name
         anom_mean_file = str(DATA_OUTmodel)+'/'+str(model_fcst[n-1])+'/anom/'+str(cyc)+'Z/'+str(reg)+'/'+str(stat_now_name)+'_mean_'+str(varname)+str(varlevel)+'.txt'
         #get number of rows
         nrow = sum(1 for line in open(anom_mean_file))
         #get number of columns
         with file(anom_mean_file) as f:
            line_header = f.readline()
            line = f.readline()
            ncol = len(line.split())
         #read data file and put in array
         data = list()
         with open(anom_mean_file) as f:
            for line in f:
         ...
         if n == 1:
            ...
            if count_nan != len(stat_now_mean):
               ...
               ax2.plot(frcst_hr, stat_now_mean - stat_exp1, color=colors[n-1], marker='o', markersize='7')
         if n == nexp:
            ax1.legend(bbox_to_anchor=(0.0, 1.02, 1.0, .102), loc=3, ncol=nexp, fontsize='13', mode="expand", borderaxespad=0.)
            if reg == 'FULL':
                 plt.savefig(str(DATA_OUTimgs_now)+'/ac_fhrmeans_'+str(varname)+'_'+str(varlevel)+'_'+str(grid)+'.png', bbox_inches='tight')
            else:
                 plt.savefig(str(DATA_OUTimgs_now)+'/ac_fhrmeans_'+str(varname)+'_'+str(varlevel)+'_'+str(grid)+str(reg)+'.png', bbox_inches='tight')
         n+=1
      s+=1  
   else:
      s+=1
