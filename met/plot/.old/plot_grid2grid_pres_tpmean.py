...
while s <= nstats: #loop over statistics
   ...
   if stat_now_name == 'ac':
      ...
      if reg == 'FULL':
         ...
         plt.savefig(str(DATA_OUTimgs_now)+'/'+str(stat_now_name)+'pmean_'+str(varname)+'_'+str(grid)+'.png', bbox_inches='tight')
      else:
         fig.suptitle(str(varname)+': '+str(stat_now_name)+'\n'+str(grid)+':'+str(reg)+' valid '+str(cyc)+'Z '+str(sday)+str(smonth)+str(syear)+'-'+str(eday)+str(emonth)+str(eyear)+' Mean\n\n', fontsize=14, fontweight='bold')
         plt.savefig(str(DATA_OUTimgs_now)+'/'+str(stat_now_name)+'pmean_'+str(varname)+'_'+str(grid)+str(reg)+'.png', bbox_inches='tight')
      s+=1
