...
while s <= nstats: #loop over statistics
   ...
   if stat_now_name == 'ac':
      ...
      while n <= nexp: #loop over experiments
         ...
         save_meanvar_file = str(DATA_OUTmodel)+'/'+str(model_fcst[n-1])+'/upper_air/'+str(cyc)+'Z/'+str(reg)+'/'+str(stat_now_name)+'_mean_'+str(varname)+str(varlevel)+'.txt'
         ...
         if n == 1:
            ...
            if nrow > 0:
               ax.plot_date(dates,stat_now_dates, color=colors[n-1], ls='-', linewidth=2.0, marker='None', label=model_fcst[n-1]+' '+str(round(np.mean(stat_now),2))+' '+str(nrow-1))
               #ax.plot_date(modeldates,stat_now, color=colors[n-1], ls='-', linewidth=2.0, marker='None', label=model_fcst[n-1]+' '+str(round(np.mean(stat_now),2))+' '+str(nrow-1))
            ax.grid(True)
            ...
            if nrow > 0:
               ax.plot_date(dates,stat_now_dates, color=colors[n-1], ls='-', marker='None', markersize=7, label=model_fcst[n-1]+' '+str(round(np.mean(stat_now),2))+' '+str(nrow-1))
               #ax.plot_date(modeldates,stat_now, color=colors[n-1], ls='-', marker='None', markersize=7, label=model_fcst[n-1]+' '+str(round(np.mean(stat_now),2))+' '+str(nrow-1))
         if n == nexp:
