...
#############################################################################
##### Read in data, compute statistic, and plot
#read in data
s=1
while s <= nstats: #loop over by statistic
   stat_now_name = plot_stats[s-1]
   if stat_now_name == 'ac':
      print '--- '+str(stat_now_name)
      n=1
      while n <= nexp: #loop over by experiment
         #get file name
         if fcsthr_in < 10:
         ...
         save_meanvar_file = str(DATA_OUTmodel)+'/'+str(model_fcst[n-1])+'/anom/'+str(cyc)+'Z/'+str(reg)+'/'+str(stat_now_name)+'_mean_'+str(varname)+str(varlevel)+'.txt'
         ...
         if n == 1:
            ...
            if nrow > 0:
               print n, model_fcst[n-1], nrow
               ax.plot_date(dates,stat_now_dates, ls='--', color=colors[n-1], marker='o', markersize='7', label=model_fcst[n-1]+' '+str(round(np.mean(stat_now),2))+' '+str(nrow-1))
               #ax.plot_date(modeldates,stat_now, ls='--', color=colors[n-1], marker='o', markersize='7', label=model_fcst[n-1]+' '+str(round(np.mean(stat_now),2))+' '+str(nrow-1))
         if n == nexp:
