...
#############################################################################
export group="anom"
##### Run MET Stat Analysis to filter anomalous partial sum files
...
while [ $vl -le $nlev ] ; do #loop over variable level
   ...
   while [ $fhr -le $fend ] ; do #loop over forecast hour
      ...
      while [ $n -le $nexp ] ; do #loop over experiment
         ...
         #variable name, variable level, and regio
         echo "------------------------------------------------------------"
...
#############################################################################
      ...
      python ${MET_HOME}/plot/plot_grid2grid_anom_ts.py
      echo "------------------------------------"
      fhr=` expr $hr + $fint`
   done
   echo "------------------------------------"
   ...
   export nstats=`echo $plot_stats_list |wc -w`
   export stats=${plot_stats[@]}
   python ${MET_HOME}/plot/plot_grid2grid_anom_tsmean.py
   echo "------------------------------------"
   echo
   echo 
   echo 
...
#############################################################################
echo "------------------------------------"
echo "----------> Plotting forecast hour-date ${region_now} ${start_date} ${end_date} c${cycle} ${var_name_now}"     
export modellist=${model_fcst[@]}
export DATA_OUTimgs_now=${DATA_OUTimgs}/${cycle}Z/${region_now}
export varname=${var_name_now}
export reg=${region_now}
...
export nstats=`echo $plot_stats_list |wc -w`
export stats=${plot_stats[@]}
python ${MET_HOME}/plot/plot_grid2grid_anom_timemap.py
echo "------------------------------------"
echo
echo 
echo
