...
while [ $n -le $nexp ] ; do #loop over experiments
   ...
   while [ $date -le $edate ] ; do #loop over date range
       #filter by initalization date using MET Stat Analysis
       echo "----------> Running Stat Analysis ${model_fcst[$nn]} valid on ${date}"
       vdate=`echo $date |head -c8 `
       stat_analysis -lookin ${DATA_OUTmodel}/${model_fcst[$nn]}/upper_air/${cycle}Z -job filter -dump_row ${DATA_OUTmodel}/${model_fcst[$nn]}/upper_air/${cycle}Z/grid2obs_v${date}.txt -fcst_valid_beg ${vdate}'_'000000 -fcst_valid_end ${vdate}'_'230000 -v ${verbose}
       echo "------------------------------------------------------------"
       date=`$ndate +24 ${date}`
   done
