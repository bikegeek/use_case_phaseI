...
while [ $n -le $nexp ] ; do #loop over experiments
   ...
   for fcycle in $fcycle_list ; do
       ...
       while [ $date -le $edate ] ; do #loop over date range
           ...
           echo "----------> Running Stat Analysis ${model_fcst[$nn]} valid on ${date} cycle ${cycle} 00Z"
           idate_e==`$ndate -fstart ${date}`
           idate_s=`$ndate -fend ${date}`
           vdate=`echo $date |head -c8 `
           stat_analysis -lookin ${DATA_OUTmodel}/${model_fcst[$nn]}/upper_air -job filter -dump_row ${DATA_OUTmodel}/${model_fcst[$nn]}/upper_air/${cycle}Z/grid2obs_v${date}.txt -fcst_int_beg ${idate_s}'_'${cycle}0000 -fcst_int_end ${idate_e}'_'${cycle}0000 -fcst_valid_beg ${vdate}'_'000000 -fcst_valid_end ${vdate}'_'230000 -v ${verbose}
           echo "------------------------------------------------------------"
           date=`$ndate +24 ${date}`
       done
   done
