...
while [ $r -le $ncsfcreg ] ; do
  rr=` expr $r - 1 `
  if [ $rr -eq 0 ]; then
     #conus_sfc_regions_path='"'"${MET_HOME}/poly/${conus_sfc_grid}_masks/${conus_sfc_regions[$rr]}.nc"'"'
     conus_sfc_regions_path='"'"${MET_HOME}/poly/${conus_sfc_regions[$rr]}.poly"'"'
  else
     #conus_sfc_regions_path[$rr]=' "'"${MET_HOME}/poly/${conus_sfc_grid}_masks/${conus_sfc_regions[$rr]}.nc"'"'
...
while [ $n -le $nexp ] ; do #loop over experiments
   ...
   if [ -n "$grid2obs_dir" ] ; then
      ...
      if [ -d ${grid2obs_dir}/conus_sfc/${cycle}Z/${model_fcst[$nn]} ] ; then
         cp ${DATA_OUTmodel}/${model_fcst[$nn]}/conus_sfc/${cycle}Z/grid2obs_i*.txt ${grid2obs_dir}/conus_sfc/${cycle}Z/${model_fcst[$nn]}/.
      else
