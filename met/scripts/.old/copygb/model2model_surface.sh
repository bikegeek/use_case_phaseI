...
#output data paths
export DATA_OUT="/global/save/Mallory.Row/VRFY/met/out_surface_copygb"
#etc
...
#use copygb to put on desired grib and puts file paths in file
GG=`echo $grid |cut -c2- `
n=1
while [ $n -le $nexp ] ; do
   nn=` expr $n - 1 `
   ...
   while [ $date -le $edate ] ; do
       for hr in $asub; do
           ${copygb} -g$GG -x ${DATA_IN_model_fcst[$nn]}/${model_fcst[$nn]}/pgb${hr}${model_fcst_dump[$nn]}${date} ${DATA_OUTmodel}/${model_fcst[$nn]}/pgb${hr}${model_fcst_dump[$nn]}${date}
           echo ${DATA_OUTmodel}/${model_fcst[$nn]}/pgb${hr}${model_fcst_dump[$nn]}${date} >> ${DATA_OUTmodel}/${model_fcst[$nn]}/${model_fcst[$nn]}_files.txt
       done
   date=`$ndate +24 ${date}`
   done
n=` expr $n + 1 `
...
echo "------------------------------------------------------------"
echo "-------------> Running copygb ${model_obs}"
date=${sdate}
while [ $date -le $edate ] ; do
   for hr in $asub; do
           ${copygb} -g$GG -x ${DATA_IN_model_obs}/${model_obs}/pgb${hr}${model_obs_dump}${date} ${DATA_OUTmodel}/${model_obs}/pgb${hr}${model_obs_dump}${date}
           echo ${DATA_OUTmodel}/${model_obs}/pgb${hr}${model_obs_dump}${date} >> ${DATA_OUTmodel}/${model_obs}/${model_obs}_files.txt
    done
    date=`$ndate +24 ${date}`
done
echo
