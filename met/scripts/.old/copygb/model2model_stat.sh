...
export plot_stats=("BIAS" "RMSE" "MSESS" "RSD" "RMSEme" "RMSEpv" "PCOR")
#output data paths
export DATA_OUT="/global/save/Mallory.Row/VRFY/met/out_gridstat_copygb"
#etc
...
#use copygb to put on desired grib
GG=`echo $grid |cut -c2- `
n=1
while [ $n -le $nexp ] ; do
   nn=` expr $n - 1 `
   echo "------------------------------------------------------------"
   echo "-------------> Running copygb ${model_fcst[$nn]}"
   date=${sdate}
   while [ $date -le $edate ] ; do
       for hr in $asub; do
           ...
           if [ -e "${DATA_IN_model_fcst[$nn]}/${model_fcst[$nn]}/pgbf${hr}${model_fcst_dump[$nn]}${fcstdate}" ] ; then
               ...
               ${copygb} -g$GG -x ${DATA_IN_model_fcst[$nn]}/${model_fcst[$nn]}/pgbf${hr}${model_fcst_dump[$nn]}${fcstdate} ${DATA_OUTmodel}/${model_fcst[$nn]}/f${hr}/pgbf${hr}${model_fcst_dump[$nn]}${fcstdate}
           fi
           ${copygb} -g$GG -x ${DATA_IN_model_fcst[$nn]}/${model_fcst[$nn]}/pgbanl${model_fcst_dump[$nn]}${date} ${DATA_OUTmodel}/${model_fcst[$nn]}/f${hr}/pgbanl${model_fcst_dump[$nn]}${date}
       done
   date=`$ndate +24 ${date}`
   done
n=` expr $n + 1 `
...
#.........................
     ...
     while [ $date -le $edate ] ; do
           for hr in $asub; do
               ...
               if [ -e "${DATA_OUTmodel}/${model_fcst[$nn]}/f${hr}/pgbf${hr}${model_fcst_dump[$nn]}${fcstdate}" ] ; then  
                  grid_stat ${DATA_OUTmodel}/${model_fcst[$nn]}/f${hr}/pgbf${hr}${model_fcst_dump[$nn]}${fcstdate} ${DATA_OUTmodel}/${model_fcst[$nn]}/f${hr}/pgbanl${model_fcst_dump[$nn]}${date} ${DATA_OUTconfig}/${model_fcst[$nn]}/GridStatConfig_${var_name[$vv]}_${var_GRIBlvltyp[$vv]}${var_level[$vv]} -outdir ${DATA_OUTmodel}/${model_fcst[$nn]}/f${hr} -log ${DATA_OUTlog}/${model_fcst[$nn]}_f${hr}_${var_name[$vv]}_${var_GRIBlvltyp[$vv]}${var_level[$vv]}.log -v ${verbose}
...
if [ $plot == yes ] ; then
   ...
   while [ $v -le $nvar ] ; do
       vv=` expr $v - 1 `
       for hr in $asub; do 
           svaliddate=`$ndate +${hr} ${sdate}`
           evaliddate=`$ndate +${hr} ${edate}`
           export modellist=${model_fcst[@]}
           export fhr=${hr}
           export DATA_OUTimgs_now=${DATA_OUTimgs}/f${hr}
           export svaliddate=${svaliddate}
           export evaliddate=${evaliddate}
