...
cat <<EOF >> ${DATA_OUTconfig}/${model_fcst[$nn]}/GridStatConfig_${var_name[$vv]}
////////////////////////////////////////////////////////////////////////////////
//
...
#.........................
       ...
       while [ $date -le $edate ] ; do
           for hr in $asub; do
               ...
               if [ -e "${DATA_IN_model_fcst[$nn]}/${model_fcst[$nn]}/pgbf${hr}${model_fcst_dump[$nn]}${fcstdate}" ] ; then
                  ...
                  if [ $vv -eq 0 ] ; then
                     echo $date >> ${DATA_OUTmodel}/${model_fcst[$nn]}/f${hr}/date_modelfile_exists.txt
                  fi
                  echo "------------------------------------------------------------"
                  echo "-------------> Running Grid Stat ${model_fcst[$nn]}, ${model_fcst[$nn]}_anl f${hr}   ${var_name[$vv]} ${var_level[@]}"
                  grid_stat ${DATA_IN_model_fcst[$nn]}/${model_fcst[$nn]}/pgbf${hr}${model_fcst_dump[$nn]}${fcstdate} ${DATA_IN_model_fcst[$nn]}/${model_fcst[$nn]}/pgbanl${model_fcst_dump[$nn]}${date} ${DATA_OUTconfig}/${model_fcst[$nn]}/GridStatConfig_${var_name[$vv]} -outdir ${DATA_OUTmodel}/${model_fcst[$nn]}/f${hr} -log ${DATA_OUTlog}/${model_fcst[$nn]}_f${hr}_${var_name[$vv]}.log -v ${verbose}
       ...
       for hr in $asub ; do
           ...
           while [ $vl -le $nlev ] ; do
             vvl=` expr $vl - 1 `
             stat_analysis -lookin ${DATA_OUTmodel}/${model_fcst[$nn]}/f${hr}/*${var_name[$vv]}* -job filter -dump_row ${DATA_OUTmodel}/${model_fcst[$nn]}/f${hr}/statanalysisCNT_${var_name[$vv]}_${var_level[$vvl]}.txt -line_type CNT -vx_mask ${reg} -fcst_lev ${var_level[$vvl]} -v ${verbose} 
             stat_analysis -lookin ${DATA_OUTmodel}/${model_fcst[$nn]}/f${hr}/*${var_name[$vv]}* -job filter -dump_row ${DATA_OUTmodel}/${model_fcst[$nn]}/f${hr}/statanalysisSL1L2_${var_name[$vv]}_${var_level[$vvl]}.txt -line_type SL1L2 -vx_mask ${reg} -fcst_lev ${var_level[$vvl]} -v ${verbose}
             vl=` expr $vl + 1 `
          done
       done
       v=` expr $v + 1 `
...
if [ $plot == yes ] ; then
   ...
   while [ $v -le $nvar ] ; do
       ...
       for hr in $asub; do
           echo "------------------------------------------------------------"
           echo 'Plotting' ${var_name[$vv]} 'f'${hr} 
           vl=1
           while [ $vl -le $nlev ] ; do
             vvl=` expr $vl - 1 `
             echo "------------------------------------"
